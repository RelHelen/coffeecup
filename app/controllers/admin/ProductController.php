...
class ProductController extends AppController {
    ...
    public function indexAction(){
        ...
        $products = \R::getAll("SELECT product.*, category.title AS cat FROM product JOIN category ON category.id = product.id_catalog ORDER BY product.title LIMIT $start, $perpage");
    ...
    public function editAction(){
      ...
      // $filter = \R::getCol('SELECT attr_id FROM attribute_product WHERE product_id = ?', [$id]);
        ...
        $modify= \R::findAll('modification', "id_product = ?", [$id]); 
       // $modify= $product->getModification($id);
        // debug($id);
        // debug($modify);
        //die;
       // $this->setMeta("Редактирование товара {$product->title}");
        $this->set(compact('product', 'related_product','modify'));
    }
